...
public class TransparentDetection : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        if (other.gameObject.GetComponent<PlayerController>())
        {
            if (spriteRenderer)
            {
                StartCoroutine(FadeRountine(spriteRenderer, fadeTime, spriteRenderer.color.a, transparencyAmount));

            } else if (tilemap)
            {
                StartCoroutine(FadeRountine(tilemap, fadeTime, tilemap.color.a, transparencyAmount));
            }
        }
    }
